  ...
  useEffect(() => {
    ...
    const handleCallAccepted = async (data) => {
      ...
      try {
        ...
        if (!answer || !answer.sdp || !answer.type) {
          console.error('[CALLER] Answer không hợp lệ:', answer);
          Alert.alert('Lỗi: Answer không hợp lệ từ phía Web.');
          handleEndCall();
          return;
        }
        console.log('[CALLER] Đang setRemoteDescription với answer:', answer);
        await peerConnectionRef.current.setRemoteDescription(
          new RTCSessionDescription(answer)
        );
        setCallStatus('connected');
      } catch (error) {
        console.error('[CALLER] Lỗi khi setRemoteDescription:', error, data.answer);
        Alert.alert('Lỗi khi thiết lập kết nối cuộc gọi. Vui lòng thử lại.');
        handleEndCall();
      }
    };
    ...
    const handleIceCandidate = async (data) => {
      ...
      try {
        ...
        // Kiểm tra peer connection
        if (!peerConnectionRef.current) {
          console.log('[MODAL] PeerConnection chưa được khởi tạo, thêm vào pending');
        ...
        if (peerConnectionRef.current.connectionState === 'closed') {
          console.log('[MODAL] PeerConnection đã đóng, bỏ qua ICE candidate');
          return;
        }
        ...
        if (!peerConnectionRef.current.remoteDescription || 
          ...
          console.log('[MODAL] Chưa có remoteDescription, thêm vào pending candidates');
          pendingCandidatesRef.current.push(candidateObj);
        } else {
          console.log('[MODAL] Đã có remoteDescription, thêm candidate ngay lập tức');
          try {
            await peerConnectionRef.current.addIceCandidate(candidateObj);
            console.log('[MODAL] Đã thêm ICE candidate thành công');
          } catch (err) {
            if (err.name === 'InvalidStateError') {
              console.log('[MODAL] PeerConnection không ở trạng thái phù hợp, thêm vào pending');
              pendingCandidatesRef.current.push(candidateObj);
            } else {
              throw err;
            }
